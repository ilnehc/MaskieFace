...
def get_line(face_landmark, image, scale, type="eye"):
    ...
    if type == "eye":
        ...
        print(face_landmark["nose_bridge"][-1][1], face_landmark["nose_bridge"][0][1])
        if scale == 0:
            mid_point = (bottom_lip_mid[0], bottom_lip_mid[1])
        elif scale == 2:
            mid_point = (bottom_lip_mid[0] + (face_landmark["nose_bridge"][0][0] - bottom_lip_mid[0]), bottom_lip_mid[1] + (face_landmark["nose_bridge"][0][1] - bottom_lip_mid[1]))
    elif type == "nose_tip":
        nose_length = (
            face_landmark["nose_bridge"][-1][1] - face_landmark["nose_bridge"][0][1]
        )
...
def fit_line(x, y, image):
    if x[0] == x[1]:
        x[0] += 0.1
    # eg. np.poly1d([1, 2, 3]) -> 1 x + 2 x + 3 -> 1*x^2 + 2*x^1 + 3*x^0
    coefficients = np.polyfit(x, y, 1)
    polynomial = np.poly1d(coefficients)
    # return 50 points from 0 to image.shape[1]
...
def get_six_points(face_landmark, image, scale):
    # nose line, nose mid point
    _, perp_line1, _, _, m = get_line(face_landmark, image, scale, type="nose_mid")
    face_b = m

    perp_line, _, _, _, _ = get_line(face_landmark, image, scale, type="perp_line")
    ...
    nose_mid_line, _, _, _, _ = get_line(face_landmark, image, scale, type="nose_long")

    angle = get_angle(perp_line, nose_mid_line)
    nose_mid_line, _, _, _, _ = get_line(face_landmark, image, scale, type="nose_tip")
    ...
    face_a = points[0]
    face_c = points[-1]
    nose_mid_line, _, _, _, _ = get_line(face_landmark, image, scale, type="bottom_lip")
    points = get_points_on_chin(nose_mid_line, face_landmark)
    face_d = points[0]
    face_f = points[-1]
...
def mask_image(image_path, args):
    ...
    for (i, face_location) in enumerate(face_locations):
        print('face_location: {}'.format(face_location))
        shape = args.predictor(gray, face_location)
        shape = face_utils.shape_to_np(shape)
        face_landmarks = shape_to_landmarks(shape)  # extract face features from shape
        face_location = rect_to_bb(face_location)
        # draw_landmarks(face_landmarks, image)
        print('face_location after transformation: {}'.format(face_location))
        six_points_on_face, angle = get_six_points(face_landmarks, image, scale)
        mask = []
        if mask_type != "all":
